...
motors <- page %>% 
 ...
 names(motors) <- c("Modell (temp. varierte fra 0C til -10C)", "WLTP-tall","STOPP", "Avvik")
...
motors$`WLTP-tall` <- str_replace(motors$`WLTP-tall`, " km", "")

motors$`WLTP-tall` <- str_replace(motors$`WLTP-tall`, " kWh", "")


motors$`WLTP-tall` <- sub("/.*", "", motors$`WLTP-tall`)

#changing my data to numeric type

motors$`STOPP` <- as.numeric(motors$`STOPP`)

motors$`WLTP-tall` <- as.numeric(motors$`WLTP-tall`)

#plotting numbers with 45 degree line 

motors %>% 
  ggplot() +
  geom_point(aes(x = `WLTP-tall`,y = STOPP))+
